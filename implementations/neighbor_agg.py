...
parser.add_argument('--bench_data_root', type=str, default='/gpfs/scratch/hc4549/HEST', help='root directory containing all the datasets')
...
parser.add_argument('--spatial_agg', action='store_true', help='Aggregate neighbor patch embeddings before regression')
parser.add_argument('--spatial_k', type=int, help='Number of nearest neighbors to aggregate (e.g. 8 for 3x3 window, 24 for 5x5)')
parser.add_argument('--spatial_agg_mode', type=str, help='How to aggregate neighbors: "mean" (same output dim) or "concat" (output dim grows to (k+1)*D, then PCA compresses)')
...
class DINOv3Wrapper(nn.Module):
    ...
    def forward(self, x):
        # DINOv3 forward returns a dict or tuple depending on version/config
        # Most implementations via hub return a dict with 'x_norm_clstoken'
        outputs = self.model(x)
        if isinstance(outputs, dict):
            return outputs['x_norm_clstoken'] # Standard global feature
        return outputs
...
class BenchmarkConfig:
    ...
    bench_data_root: Optional[str] = ''
    # Benchmark data will automatically be downloaded to this path

    embed_dataroot: Optional[str] = './HEST/AggMean_data_emb'
    # Embeddings generated during benchmarking will be saved to this path

    results_dir: Optional[str] = './HEST/AggMean_pred_results'
    ...
    spatial_agg: bool = False
    spatial_k: int = 8
    spatial_agg_mode: str = 'concat'
...
def neighbor_aggregate(
    ...
    """Aggregate each spot's embedding with its k nearest spatial neighbors.

    Args:
        embeddings: (N, D) patch embeddings for a single sample.
        coords: (N, 2) pixel coordinates for the same sample.
        k: number of neighbors (8 = 3x3 window minus center, 24 = 5x5 minus center).
        mode: 'mean' keeps output shape (N, D); 'concat' produces (N, (k+1)*D).

    Returns:
        Aggregated embeddings array.
    """
    tree = KDTree(coords)
    # k+1 because query returns the point itself as the closest neighbor
    actual_k = min(k + 1, len(embeddings))
    _, indices = tree.query(coords, k=actual_k)
    neighbor_indices = indices[:, 1:]  # drop self; shape (N, actual_k-1)

    neighbor_embs = embeddings[neighbor_indices]  # (N, actual_k-1, D)

    if mode == 'mean':
        center = embeddings[:, np.newaxis, :]  # (N, 1, D)
        return np.concatenate([center, neighbor_embs], axis=1).mean(axis=1)
    elif mode == 'concat':
        neighbor_flat = neighbor_embs.reshape(len(embeddings), -1)  # (N, (actual_k-1)*D)
        return np.concatenate([embeddings, neighbor_flat], axis=1)
    else:
        raise ValueError(f"Unknown spatial_agg_mode '{mode}'. Use 'mean' or 'concat'.")
...
def predict_single_split(train_split, test_split, args, save_dir, dataset_name, model_name, device, bench_data_root, custom_encoder, extract_tiles):
    ...
    if custom_encoder is not None and model_name == getattr(custom_encoder, '_hest_name', None):
        encoder = custom_encoder
        args.overwrite = True 
    else:
        # Only call the factory for built-in models (like uni_v2)
        encoder = encoder_factory(model_name)
    ...
    for split_key, split in zip(['train', 'test'], [train_df, test_df]):
        ...
        for i in tqdm(range(len(split))):
            ...
            if args.spatial_agg:
                assets['embeddings'] = neighbor_aggregate(
                    assets['embeddings'],
                    assets['coords'],
                    k=args.spatial_k,
                    mode=args.spatial_agg_mode,
                )
...
def benchmark(
    ...
    **kwargs
    ...
    if encoder is not None:
        ...
        current_name = cli_args.model_name if cli_args else 'custom_encoder'
        encoders.append(current_name)
        ...
        if hasattr(encoder, "eval_transforms") and hasattr(encoder, "precision"):
            ...
            custom_encoder = CustomInferenceEncoder(current_name, encoder, enc_transf, precision)
        custom_encoder._hest_name = current_name
    else:
        custom_encoder = None
...
if __name__ == '__main__':
    ...
    # 1. Logic to decide which path to take
    if cli_args.model_name == 'uni_v2':
        # TRIDENT native route: pass uni_v2 in the encoders list
        benchmark(
            encoder=None,      # No custom object needed
            enc_transf=None, 
            precision=None,
            cli_args=cli_args,
            encoders=['uni_v2'], # Trident will handle this automatically
            datasets=datasets_hest,
            exp_code='uni_v2'
        )
        ...
        wrapped_model = DINOv3Wrapper(raw_model)
        dinov3_transf = make_dinov3_transform(224)
        
        benchmark(
            encoder=wrapped_model,
            enc_transf=dinov3_transf,
            precision=torch.float32,
            cli_args=cli_args,
            encoders=[],
            datasets=datasets_hest,
            exp_code=cli_args.model_name
        )
